//============================================================================
// RPCA_Topic_Model.cpp
//
// Performs topic modeling using Tensor RPCA. This code is based on
// "Single Node ALS Topic Modeling" by Furong Huang.
//============================================================================

#include "util/stdafx_dd.h"
...
int main(int argc, const char * argv[])
{
	...
	// e.g. $(SolutionDir)\datasets\$(CorpusName)\samples_train.txt $(SolutionDir)\datasets\$(CorpusName)\samples_test.txt
	const char* FILE_T_WRITE     = "datasets/news/result_RPCA/matrix_for_RPCA.txt";
	const char* FILE_alpha_WRITE = argv[9];
	const char* FILE_beta_WRITE  = argv[10];
	const char* FILE_hi_WRITE    = argv[11];
	// (3) Output files
	// FILE_T_WRITE denotes the filename for the tensor decomposed using Tensor RPCA.
	//		The KHIDxKHIDxKHID tensor is stored as a KHIDxKHID^2 matrix.
	...
	// The format is:
	// $(SolutionDir)\datasets\$(CorpusName)\result_RPCA\alpha.txt
	// $(SolutionDir)\datasets\$(CorpusName)\result_RPCA\beta.txt
	// $(SolutionDir)\datasets\$(CorpusName)\result_RPCA\hi.txt
	// e.g. $(SolutionDir)\datasets\$(CorpusName)\result_RPCA\alpha.txt $(SolutionDir)datasets\$(CorpusName)\result_RPCA\beta.txt $(SolutionDir)datasets\synthetic\result_RPCA\hi.txt
	...
	if (M3_yes){
		input_M3 = read_G_sparse((char*) FILE_M3, "third order moment", KHID, KHID * KHID);
	}
  ...
  int whichRun = 0;
	...
	cout << "\nWriting tensor to be decomposed using Tensor RPCA--------" << endl;
	write_beta((char *)FILE_T_WRITE, T);

	TIME_start = clock();
	cout << "\nStarting MATLAB to run Tensor RPCA--------" << endl;
	char * exe_str = "/Applications/MATLAB_R2017b.app/bin/matlab -nosplash -nodesktop -r \"run(\'util/decomp_for_topic_modeling\')\"";
	system(exe_str);

  cout << "\nFinished Tensor RPCA--------" << endl;
	cout << "\nWriting factor matrices--------" << endl;

	phi_new = readMatrix((char *) "datasets/news/result_RPCA/phi.txt");
	lambda  = readMatrix((char *) "datasets/news/result_RPCA/lambda.txt");

	TIME_end = clock();
	double time_stpm = double(TIME_end - TIME_start) / CLOCKS_PER_SEC;
	printf("time taken by Tensor RPCA = %5.10e (Seconds)\n", time_stpm);
	...
	cout << "\nWriting topic model results-------" << endl;
